...
// Check if user has specific role in a team
...
// Check if user can manage team (create/edit tasks and projects)
// Returns true if user has Admin or Project Manager role in the team
export async function canManageTeam(userId, teamId) {
  if (!teamId) return false;
  return await hasTeamRole(userId, teamId, ['Admin', 'Project Manager']);
}

// Check if user can create/edit tasks for a team
export async function canCreateTasks(session, teamId = null) {
  if (!teamId) return false;
  return await canManageTeam(session.user.id, teamId);
}

// Check if user can create/edit projects for a team
